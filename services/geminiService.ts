...
const getSystemInstruction = (files: Files): string => {
...
export const generateContentFromChat = async (history: Message[], files: Files, apiKey: string): Promise<{ message: string; filesToUpdate: Record<string, string>, roadmap?: string[], commandsToExecute?: string[] }> => {
  if (!apiKey) {
    return {
  ...
  const systemInstruction = getSystemInstruction(files);
